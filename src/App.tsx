  ...
  const sendMessageToBackend = async (
    ...
    try {
      ...
      const completeBotResponse = async (responseData: any, isError = false) => {
        ...
        if (isError) {
           const fallbackMessage = typeof responseData === 'string' && responseData.trim() ? responseData : "Sorry, but Models have reached the free token usage limit. Please try again in 5 minutes.";
           ...
           if ((!rawBotText || !rawBotText.trim()) && !botImage && !botAudio) {
               rawBotText = "Sorry, but Models have reached the free token usage limit. Please try again in 5 minutes.";
           }
      ...
      try {
        ...
        if (err.name === 'AbortError') {
             ...
             setTimeout(() => {
                 if (!isResolved) {
                     completeBotResponse("Sorry, but Models have reached the free token usage limit. Please try again in 5 minutes.", true);
                 }
             }, 1000);
        }
      }
      ...
      const errorMsg = { id: generateUniqueId(), text: "Sorry, but Models have reached the free token usage limit. Please try again in 5 minutes.", sender: 'bot', timestamp: new Date() };
