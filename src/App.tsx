    ...
    if (part.startsWith('`') && part.endsWith('`') && part.length >= 2) {
      return (
        <code key={i} className={`px-1.5 py-0.5 mx-0.5 rounded-md text-[0.86em] font-mono border whitespace-nowrap inline-block max-w-full overflow-x-auto align-middle transition-colors backdrop-blur-md ${
          isDarkMode ? 'bg-sky-950/40 text-cyan-200 border-cyan-400/40 shadow-[inset_0_1px_1px_rgba(255,255,255,0.35)]' : 'bg-blue-100/80 text-blue-900 border-blue-300/70'
        }`}>
          {part.slice(1, -1)}
        </code>
      );
              ...
              <div className="relative flex-shrink-0" ref={attachMenuRef}>
                {showAttachMenu && (
                  <div 
                    className={`absolute bottom-full left-0 mb-3 w-56 sm:w-64 p-2 rounded-2xl border shadow-2xl backdrop-blur-3xl z-50 animate-float-up ${
                      isDarkMode 
                      ? 'bg-[#09152a]/60 border-cyan-400/40 text-slate-100 shadow-[0_20px_50px_rgba(0,0,0,0.6),inset_0_1.5px_2px_rgba(255,255,255,0.4)]' 
                ...
                <button
                      ...
                      className={`p-2.5 sm:p-3 rounded-full transition-all flex items-center justify-center ${
                        showAttachMenu
                        ? (isDarkMode ? 'bg-cyan-400/30 text-cyan-200 border border-cyan-300/50' : 'bg-blue-100 text-blue-700')
                        : (isDarkMode ? 'text-cyan-300 hover:text-white hover:bg-cyan-400/20' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-100')
                      }`}
                  ...
                  <div className="relative flex-1 flex flex-col justify-end min-w-0">
                    {activeTool && (
                      <div className="px-2 pt-1 pb-1 flex items-center animate-float-up">
                        <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[12px] font-medium border shadow-sm ${isDarkMode ? 'bg-sky-500/25 text-cyan-200 border-cyan-400/40 shadow-[inset_0_1px_1px_rgba(255,255,255,0.35)]' : 'bg-blue-50 text-blue-600 border-blue-200'}`}>
                    ...
                    <textarea
                      ...
                      className={`w-full max-h-48 min-h-[38px] sm:min-h-[46px] px-2 sm:px-3 bg-transparent outline-none resize-none text-[15px] ${activeTool || pendingAttachment ? 'pt-1 pb-2' : 'py-2 sm:py-3'} ${isDarkMode ? 'text-slate-100 placeholder-cyan-200/50' : 'text-slate-900 placeholder-slate-500'}`}
                      rows={1}
                    />
                  </div>
                  ...
                  <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0 pr-1">
                    ...
                    {inputValue.trim() || pendingAttachment ? (
                      <button
                        ...
                        disabled={isLoading}
                        title="Send Message"
                        className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-sky-500/30 hover:bg-sky-400/50 text-cyan-200 border border-sky-300/50 shadow-[inset_0_1.5px_1.5px_0_rgba(255,255,255,0.4)] backdrop-blur-xl active:scale-95 disabled:opacity-50 flex-shrink-0 flex items-center justify-center transition-all cursor-pointer"
                      >
                      ...
                      <button
                        ...
                        className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center transition-all backdrop-blur-xl active:scale-95 flex-shrink-0 ${
                          isRecording
                          ? 'bg-sky-500/40 text-cyan-100 border border-cyan-300/60 animate-pulse shadow-[inset_0_1.5px_2px_0_rgba(255,255,255,0.5)]'
                          : (isDarkMode ? 'bg-sky-950/40 text-cyan-200 border border-cyan-400/40 hover:bg-sky-400/30 shadow-[inset_0_1px_1px_rgba(255,255,255,0.3)]' : 'bg-slate-200/80 text-slate-700 hover:bg-slate-300')
                        }`}
                      >
